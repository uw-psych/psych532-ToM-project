...
gender <- read.csv("../behavioral_data/ToMBooklet2_gender.csv") %>%
  rename(subID = SubID)
```
...
cluster_gender_differences$Cluster <- as.numeric(cluster_gender_differences$Cluster)

cluster_gender_differences <- cluster_gender_differences %>%
  filter(!is.na(Cluster)) %>%
  arrange(Cluster)

ggplot(cluster_gender_differences, aes(x = factor(Cluster, levels = unique(Cluster)), y = gender_difference, fill = "Gender Difference")) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
