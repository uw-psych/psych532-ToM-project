...
```{r}
tom2 <- read.csv("../behavioral_data/ToMBooklet1-Data.csv")

gender <- read.csv("../behavioral_data//ToMBooklet1_gender.csv") %>%
  rename(subID = SubID) %>%
  mutate(Age = as.integer(Age)) # age into interger
...
tom2_gender <- right_join(tom2, gender, by = "subID") %>%  # right_join
  group_by(Question_ID, Gender) # group_by
...
unique_ages <- sort(unique(tom2_gender$Age)) # with diffferent age
...
p1 <- ggplot(data) +
  ...
  scale_color_manual(name = "Mean/SD", values = c("Mean" = "blue", "SD" = "red")) +
  scale_linetype_manual(name = "Mean/SD", values = c("Mean" = "solid", "SD" = "dashed")) +
...
p2 <- ggplot(data) +
  ...
  labs(title = "Gender by Age", x = "Age", y = "ToM score") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(min(data$Age), max(data$Age), by = 1)) +
  theme(plot.title = element_text(size = 20),
        legend.title = element_text(size = 16),  # Adjust legend title size
...
# Save combined plot
ggsave("combined_plot.png", combined_plot, width = 16, height = 8, units = "in")
```
...
## use gender for counts
age_gender_counts <- gender %>%
  filter(Age >= 1 & Age <= 12) %>%
  group_by(Age, Gender) %>%
...
ggplot(age_gender_counts, aes(x = factor(Age), y = count, fill = Gender)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Age and Gender Distribution", x = "Age", y = "Count") +
  theme_minimal() +
  scale_fill_manual(values = c("Female" = "pink", "Male" = "lightblue")) +
  scale_x_discrete(limits = as.character(5:11)) +
   theme(
...
print(unique_ages)
for (age in unique_ages_11) {  # exclude 12
# gender difference code 
  older <- tom2_gender %>%
    filter(Age == age)
  print(age)
  ...
  #print(paste("Summary for Age", age, "and up:")) #print seperately 
  #print(older_summary)
...
older_differences <- older %>%
  ...
  #print(paste("Differences for Age", age, "and up:"))# print seperatly
  #print(older_differences)

  #Question analysis based on age
